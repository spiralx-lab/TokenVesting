...
contract TeamVesting is Ownable {
    ...
    uint256 public constant CYCLE_TIMES = 360 * 2;
    ...
    function getPendingReward(address _account) public view returns (uint256) {
        ...
        uint256 userReward = users[_account].weight.mul(totalAmount).mul(cycle).div(CYCLE_TIMES).div(totalWeight);
        return userReward.sub(users[_account].historyAmount);
    }
